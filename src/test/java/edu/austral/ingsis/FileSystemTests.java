  ...
  public void test1() {
    executeTest(
        List.of(
            ...
            entry("ls --ord=desc", "emily horace") // entry("ls --ord=asc", "emily horace")
            ));
  }
  ...
  void test2() {
    executeTest(
        List.of(
            ...
            entry("ls", "elizabeth.txt t-bone") // entry("ls", "elizabeth t-bone")
            ));
  }
  ...
  void test3() {
    executeTest(
        List.of(
            ...
            entry(
                "rm t-bone",
                "cannot remove 't-bone', is a directory"), // entry("rm", "cannot remove 't-bone',
            // is a directory"),
  ...
  void test8() {
    executeTest(
        List.of(
            ...
            entry("touch jetta.txt", "'jetta.txt' file created"),
            entry("ls", "emily horace.txt jetta.txt"), // entry("ls", "emily emily.txt jetta.txt"),
            entry("rm --recursive emily", "'emily' removed"),
            entry("ls", "horace.txt jetta.txt"), // entry("ls", "emily.txt jetta.txt"),
            entry(
                "ls --ord=desc",
                "jetta.txt horace.txt") // entry("ls --ord=desc", "jetta.txt emily.txt")
            ));
  }
}
